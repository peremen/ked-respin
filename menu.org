...
submenu::submenu(container* parent, submenu_info* asmi, int x, int y) :
       (parent, ExposureMask|ButtonMotionMask|LeaveWindowMask, 0, False, 0,
	True)
{
  smi = asmi;
...
void submenu::motion(XEvent *event)
{
      ...
      if (event->xmotion.y > y && event->xmotion.y < y + h)
	{
	  ...
	  if (active)
	    XFillRectangle(display(), window(), igc(), 1, oldy,
			   width() - 3, h);
	  XFillRectangle(display(), window(), igc(), 1, y, width() - 3, h);
	  active = i;
	  oldy = y;
	  return;
	}  
  ...
  if (active)
    {
      XFillRectangle(display(), window(), igc(), 1, oldy, width() - 3, h);
      active = 0;
    }
}

void submenu::leave(XEvent* event)
{
  if (active)
    {
      XFillRectangle(display(), window(), igc(), 1, oldy, width() - 3, h);
